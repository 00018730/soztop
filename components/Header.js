...
export default function Header({ onHelp, onRestart, showDailyBadge = true }) {
  ...
  useEffect(() => {
    const d = new Date();
    setDateLabel(`${d.getDate()}-${UZ_MONTHS[d.getMonth()]}`);
  }, []);
    ...
    <header className="flex items-center justify-between gap-3 py-3">
      <div className="flex items-center gap-3">
        ...
        <div>
          ...
          <p className="text-xs text-text-dim mt-1">5 harf. 1 soʻz. Sen top!</p>
        </div>
      </div>
      ...
      <div className="flex items-center gap-3">
        ...
        <button
          ...
          className="w-9 h-9 rounded-lg bg-surface border border-border flex items-center justify-center text-text-dim hover:text-text transition-colors"
        >
          ↻
        </button>
        <button
          ...
          onClick={onHelp}
          aria-label="Qoidalar"
          className="w-9 h-9 rounded-lg bg-surface border border-border flex items-center justify-center font-bold text-text-dim hover:text-text transition-colors"
        >
          ?
        </button>
        <ThemeToggle />
