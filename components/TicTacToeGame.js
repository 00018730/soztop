...
import { useTicTacToeOnline } from "@/lib/useTicTacToeOnline";
...
export default function TicTacToeGame({ registerControls }) {
  const [tab, setTab] = useState("computer");
  const local = useTicTacToeLocal();
  ...
  return (
    <>
      <GameCardHeader
        icon="⭕"
        title="Krestik-nolik"
        subtitle="Uch xilni qatorga tering — kompyuterga qarshi, ikkovlashib yoki onlayn."
      />
...
function LocalBoard({ local }) {
  ...
  if (local.status === "over") {
    if (local.result.winner === "draw") {
      ...
      statusText = `${local.result.winner.toUpperCase()} yutdi!`;
    }
  } else if (local.mode === "computer") {
    statusText = local.turn === "x" ? "Sizning navbatingiz (X)" : "Kompyuter oʻylamoqda...";
  } else {
    ...
    <>
      ...
      <div className="flex gap-2">
        {local.mode === "computer" ? (
          ...
          <>
            <Stat n={local.stats.played} l="OʻYIN" />
            <Stat n={local.stats.xWins} l="X GʻALABA" />
            <Stat n={local.stats.oWins} l="O GʻALABA" />
            <Stat n={local.stats.draws} l="DURRANG" />
          </>
        )}
      </div>
    ...
    return (
      <div className="bg-surface border border-border rounded-2xl p-6 text-center flex flex-col gap-4">
        <p className="text-sm text-text-dim">
          Tasodifiy raqib bilan jonli oʻynang — navbat bilan X va O qoʻyasiz.
        </p>
        <button
          type="button"
  ...
  return (
    <>
      <p className="text-xs text-text-dim text-center">
        Siz — <b className="text-text">{mySymbol?.toUpperCase()}</b>
      </p>
      <p className="text-sm font-bold text-center">{statusText}</p>
