...
import { checkWinner, computerMove, emptyBoard, validColumns } from "./connectFour";
...
function statsKey(mode, difficulty) {
  return mode === "computer" ? `soztop-c4-stats-computer-${difficulty}` : "soztop-c4-stats-local";
}
...
export function readConnectFourTotalPlayed() {
  return (
...
function reducer(state, action) {
  switch (action.type) {
    ...
    case "PLAY": {
      ...
      const { col } = action;
      if (!validColumns(state.board).includes(col)) return state;
      for (let r = state.board.length - 1; r >= 0; r--) {
        if (state.board[r][col] == null) {
          const board = state.board.map((row) => row.slice());
          board[r][col] = state.turn;
          const result = checkWinner(board);
          if (result) return { ...state, board, status: "over", result };
...
export function useConnectFourLocal() {
  ...
  useEffect(() => {
    ...
    const t = setTimeout(() => {
      const col = computerMove(state.board, "o", state.difficulty);
  ...
  const play = useCallback((col) => dispatch({ type: "PLAY", col }), []);
  const newGame = useCallback(() => dispatch({ type: "NEW_GAME" }), []);

  // Only meaningful for a hard-mode win against the computer — there's no
  // daily index here to compare against, so the only shareable moment is
  // "I beat the hard AI," not an arbitrary local/easy result.
  const shareText = useCallback(
    () => "🔴 Kompyuterni Toʻrt ketma-ketda (qiyin) yutdim! 🎉",
    []
  );
